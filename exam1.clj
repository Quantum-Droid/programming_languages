...
(defn gcd
  "Calculating GDC to aid lcm function."
  [a b]
  (if (= a b) a
    (if (> a b) (gcd (- a b) b)
      (gcd a (- b a)))))
...
(defn next-pascal
  "Produces a pascal row."
  [n prev-row]
  (cons 1
    (loop [n n
           l prev-row
           res ()]
      (if (= n 1) (reverse (cons 1 res))
        (recur (dec n) (rest l) (cons (+ (first l) (second l)) res))))))
...
(defn pascal
  ...
  (cond
    ...
    (= n 1) '(1 1)
    (= n 2) '(1 2 1)
    (> n 2) (next-pascal n (pascal (dec n)))))
